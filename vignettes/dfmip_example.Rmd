...
First: Load the ArboMAP and dfmip packages. If they are not installed, they will be installed from GitHub

```{r LoadPackages, echo = TRUE, message = FALSE}

if (!require(ArboMAP)){
  #install.packages("C:/Users/ak697777/University at Albany - SUNY/Elison Timm, Oliver - CCEID/CODE/ArboMAP/ArboMAP/ArboMAP_0.1.0.tar.gz", repos = NULL)
  devtools::install_github("akeyel/ArboMAP/ArboMAP_package", ref = "package_test")
  library(ArboMAP) # !require can't pull ArboMAP, so it will return a message "No Package called ArboMAP" because it looks for it on CRAN
}
if (!require(dfmip)){
  #install.packages("C:/Users/ak697777/University at Albany - SUNY/Elison Timm, Oliver - CCEID/CODE/dfmip_0.1.0.tar.gz", repos = NULL)
  devtools::install_github("akeyel/dfmip")
  library(dfmip)
}

#**# For troubleshooting
# devtools::reload('dfmip') # Must be in directory containing dfmip. This installs from a local directory, NOT github. #**# MAYBE NOT - still getting same error.
# install.packages('C:/Users/ak697777/University at Albany - SUNY/Elison Timm, Oliver - CCEID/CODE/dfmip', repos = NULL, type = 'source')
# # Restart R, but restore packages and objects (uses new versions of packages)
# #.rs.restartR()
...
models.to.run = c("RF1_C","ArboMAP", "NULL")
...
models.to.run = c("RF1_C")
dfmip.outputs = dfmip.forecast(models.to.run, human.data, mosq.data, weather.data,
...
# Define what should be hindcast
hindcast.targets = c("human_cases", "seasonal_mosquito_MLE")

# Remaining inputs were defined above
hindcasts = dfmip.hindcasts(models.to.run, focal.years, hindcast.targets, human.data, mosq.data,
                           weather.data, districtshapefile, weekinquestion, week.id,
                           results.path, arbo.inputs = arbo.inputs,
                          population.df = population.df, rf1.inputs = rf1.inputs,
                          threshold = 1, percentage = 0.25)
#**# NEED TO DEAL WITH forecast.targets
# forecast.target = "human_cases"
