...
#' Run Disease Forecast Model Intercomparison Project (dfmip)
#'
#' A single function designed to provide a simple, consistent interface to multiple disease forecasting models.
...
#'
#' @param forecasts.df A data frame that contains prior model results (e.g. for
#' a different week, state or model). If there are no prior results, this
#' should be initialized as NA and it will be generated by dfmip
#' @param models.to.run A string vector of the models to run. Options are
#' NULL.MODELS, ArboMAP, ArboMAP.MOD, RF1. Note these entries are
#' case-sensitive. NULL.MODELS make forecasts based on statewide incidence. Future versions may support district-specific results as well
#' ArboMAP returns the original ArboMAP forecasts, see details for citations
#' ArboMAP.MOD produces modified ArboMAP forecasts for annual and weekly number of human cases
#' RF1 applies the Random Forest method used in Keyel et al. 2019 PLOS ONE
#' Note that the models are run by keyword, so run in a fixed order (NULL.MODELS, ArboMAP, ArbMAP.MOD, RF1),
...
#'
#' @export run.dfmip
run.dfmip = function(forecasts.df, models.to.run, human.data, mosq.data,
  ...
  # Run the Random Forest 1 model
  if ("RF1" %in% models.to.run){
